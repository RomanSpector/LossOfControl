...
function LossOfControlFrame_OnEvent(event, self, ...)
    ...
    elseif ( event == "LOSS_OF_CONTROL_ADDED" and ... ) then
        ...
        if ( data.displayType == DISPLAY_TYPE_ALERT ) then
            ...
            or ( priority == self.priority and timeRemaining
            and ( not self.TimeLeft.timeRemaining or timeRemaining > self.TimeLeft.timeRemaining ) ) ) then
...
function LossOfControlFrame_UpdateDisplay(self)
    ...
    if ( data and data.displayText and data.displayType == DISPLAY_TYPE_FULL ) then
        ...
        if ( not self.Anim:IsPlaying() and data.startTime ) then
            CooldownFrame_SetTimer(self.Cooldown, data.startTime, data.duration, 1);
        end
